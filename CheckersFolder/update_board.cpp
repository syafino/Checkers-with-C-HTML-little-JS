...
int main() {
    try {
	    ...
	    // **Corrected check for game state using getGameState()**
	    if (gameBoard.getGameState() != ONGOING) {
    		cerr << "Game is not ongoing. Ending move." << endl;

    		// Determine the winner based on the game state
    		string winnerMessage;
    		if (gameBoard.getGameState() == WHITE_WIN) {
        		winnerMessage = "White wins!";
    		} else if (gameBoard.getGameState() == BLACK_WIN) {
        		winnerMessage = "Black wins!";
    		} else if (gameBoard.getGameState() == DRAW) {
        		winnerMessage = "The game is a draw!";
    		}

    		// Output the winner message to the HTML
    		cout << "<div id='board'>" << winnerMessage << "</div>";
    		cout << "<input type='hidden' id='currentBoardState' value=''>";
    		return 0; // Stop if the game is not ongoing
	    }
