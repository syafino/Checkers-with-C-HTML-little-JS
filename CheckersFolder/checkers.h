...
class Piece{
	public:
        enum Type { NONE, BLACK, WHITE, BKING, WKING };
...
class Board{
	...
	static bool isWhiteTurn;  // Add this member

	public:
    	Board() : row(BOARD_SIZE), column(BOARD_SIZE), currentState(ONGOING) {
	...
	bool kingable(int row, int col);
	bool kingvalid_move(int fromRow, int fromCol, int toRow, int toCol);
	bool checkmate();
	void interface();
